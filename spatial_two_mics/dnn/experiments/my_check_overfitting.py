...
naive experimental setup for checking the capability of the model to
verfit on a set of data
...
def train(args,
    ...
    bar = ChargingBar("Training for epoch: {}...".format(epoch),
                      max=n_batches)
    ## debug
    print('***GOT HERE, pre batch data***', training_generator)
    ##
    for batch_data in training_generator:
        ## debug
        print('***GOT HERE, BATCH DATA***', batch_data)
        ##
        ...
        if torch.cuda.is_available():
            ...
            flatened_ys = one_hot_ys.view(one_hot_ys.size(0), -1, one_hot_ys.size(-1))
...
def convergence_of_LSTM(args):
    ...
    val_generator, mean_val, std_val, n_val_batches = \
    ...
    for epoch in np.arange(args.epochs):
        ...
        train(args, model, val_generator, optimizer, mean_val,
              std_val, epoch, history, n_val_batches)
        ...
        if epoch % args.evaluate_per == 0:
            eval(args, model, val_generator, mean_val,
                 std_val, epoch, history, n_val_batches, k_means_obj)
